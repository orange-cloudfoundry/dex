...
func newTestServer(t *testing.T, updateConfig func(c *Config)) (*httptest.Server, *Server) {
	...
	config := Config{
		...
		AllowedGrantTypes: []string{ // all implemented types
			...
			grantTypeImplicit,
			grantTypePassword,
			grantTypeExternalClientCredentials,
		},
		Signer: sig,
	}
	if updateConfig != nil {
...
func TestServerSupportedGrants(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:      "Simple",
			config:    func(c *Config) {},
			resGrants: []string{grantTypeAuthorizationCode, grantTypeExternalClientCredentials, grantTypeRefreshToken, grantTypeDeviceCode, grantTypeTokenExchange},
		},
		{
			name:      "Minimal",
		...
		},
		{
			...
			resGrants: []string{grantTypeAuthorizationCode, grantTypeExternalClientCredentials, grantTypePassword, grantTypeRefreshToken, grantTypeDeviceCode, grantTypeTokenExchange},
		},
		{
			name:      "With token response",
			config:    func(c *Config) { c.SupportedResponseTypes = append(c.SupportedResponseTypes, responseTypeToken) },
			resGrants: []string{grantTypeAuthorizationCode, grantTypeExternalClientCredentials, grantTypeImplicit, grantTypeRefreshToken, grantTypeDeviceCode, grantTypeTokenExchange},
		},
		{
			name: "All",
			config: func(c *Config) {
			...
			resGrants: []string{grantTypeAuthorizationCode, grantTypeExternalClientCredentials, grantTypeImplicit, grantTypePassword, grantTypeRefreshToken, grantTypeDeviceCode, grantTypeTokenExchange},
		},
	}
